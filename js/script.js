...
function Vergelijk() {
  ...
  divVergelijkResult.innerHTML = `a (${a}) == b (${b}) => ${a == b} <br/>
  a (${a}) == c (${c}) => ${a == c} <br/>
  a (${a}) === b (${b}) => ${a === b} <br/>
  b (${b}) === d (${d}) => ${b === d} <br/>
  a (${a}) === c (${c}) => ${a === c} <br/>
...
function Wiskunde() {
  ...
  divWiskundeResult.innerHTML = `a (${a}) + b (${b}) => ${a + b} <br/>
  a (${a}) - b (${b}) => ${a - b} <br/>
  a (${a}) * b (${b}) => ${a * b} <br/>
  a (${a}) / b (${b}) => ${a / b} <br/>
  a (${a}) % b (${b}) => ${a % b} <br/>
...
function BoolVergelijking() {
  ...
  divBoolResult.innerHTML = `(a (${a}) < 10) && (b(${b}) > 40) => ${
    a < 10 && b > 40
  } <br/>
...
function IfUitvoer() {
  ...
  if (prijs < 15)
    resultString = `De cursus met prijs & euro; ${prijs} is niet zo duur.`;
  ...
  aantaalStudenten < maxStudentPerGroup
    ? (divIfResult.innerHTML = resultString += `<br/> Er zijn nog ${
        maxStudentPerGroup - aantaalStudenten
      } plaatsen over`)
...
function SwitchUitvoer() {
  ...
  ingaveCorrect
    ? (resultString = " is een scheepvaarder")
    : (resultString = "is geen scheepvarder");
...
function BreakDemo() {
  ...
  divBreakResult.innerHTML = resultString;
}
